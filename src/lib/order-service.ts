...
import { MEDUSA_BACKEND_URL, ELECTRONICS_STORE_KEY, HEALTH_STORE_KEY } from './config';
 ...
 */
function getPublishableKey(storeType: 'electronics' | 'health' = 'electronics'): string {
 ...
 */
export async function completeCart(request: CreateOrderRequest): Promise<OrderResponse> {
  try {
    ...
    const paymentResponse = await fetch(`${MEDUSA_BACKEND_URL}/store/carts/${cartId}/payment-collections`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-publishable-api-key': publishableKey,
    ...
    const completeResponse = await fetch(`${MEDUSA_BACKEND_URL}/store/carts/${cartId}/complete`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-publishable-api-key': publishableKey,
